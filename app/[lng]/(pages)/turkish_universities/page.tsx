...
export default async function UniversitiesPage({
  ...
  return (
    <div className="my-10 container mx-auto space-y-20 p-4">
      ...
      <div>
        <ul className="flex gap-4 [&>li]:pb-2 overflow-x-auto hidden-scrollbar text-base md:text-lg">
          <li className="border-b-2 border-red-500 text-red-500">
            <LinkApp href="/blogs" lng={lng}>{t('links.istanbulEuropean')}</LinkApp>
          </li>
          <li>
            <LinkApp href="/blogs"  lng={lng}>{t('links.istanbulAsian')}</LinkApp>
          </li>
          <li>
            <LinkApp href="/blogs" lng={lng}>{t('links.ankara')}</LinkApp>
          </li>
          <li>
            <LinkApp href="/blogs" lng={lng}>{t('links.mersin')}</LinkApp>
          </li>
          <li>
            <LinkApp href="/blogs" lng={lng}>{t('links.gaziantep')}</LinkApp>
          </li>
          <li>
          ...
          <div className=" bg-white rounded-xl grid grid-cols-1 gap-4 lg:grid-cols-2 h-full shadow-xl p-3 ">
            <CarouselItem className="order-2 lg:order-1 flex flex-col justify-center text-start text-base space-y-4 md:px-10">
              <div className="flex gap-x-4 items-center">
                <Image src={img1} alt="img" width={300} height={300} className="w-24 h-24 rounded-full" />
                <h2 className="text-lg font-bold text-gray-500">{t('universityInfo.name')}</h2>
              </div>
              ...
              <div>
                ...
                <ul className="flex gap-x-4 text-base text-gray-500">
                  <li className="list">{t('universityInfo.majors.humanSciences')}</li>
                  <li className="list">{t('universityInfo.majors.pharmacy')}</li>
                  <li className="list">{t('universityInfo.majors.computerEngineering')}</li>
                  <li className="list">{t('universityInfo.majors.medicine')}</li>
                </ul>
              </div>
              <div className="flex gap-8">
                <b className="text-red-500">{t('universityInfo.startingPrices')} : </b>
                <span className="flex gap-x-2 text-sm">
                  <span>{t('universityInfo.priceRange.from')}</span> <span>100$</span></span>
                <span className="flex gap-x-2 text-sm">
                  <span>{t('universityInfo.priceRange.to')}</span> <span>1000$</span></span>
              </div>
              <div className="flex justify-center">
                <Button color="primary" className="bg-primary text-white hover:bg-white hover:text-primary">{t('universityInfo.registerNow')}</Button>
              </div>
              {/* <h1 className="text-lg font-bold text-gray-500 md:text-2xl">
            ...
            <div className={`hidden lg:block order-1 lg:order-2 h-52 lg:h-full bg-center bg-cover lg:bg-cover`}>
              <Image src={img1} alt="img" width={300} height={300} className="w-full h-full object-cover rounded-xl" />
            </div>
          </div>
          <div className=" bg-white rounded-xl grid grid-cols-1 gap-4 lg:grid-cols-2 h-full shadow-xl p-3 ">
            <CarouselItem className="order-2 lg:order-1 flex flex-col justify-center text-start text-base space-y-4 md:px-10">
      ...
      <div className="flex justify-center">
        <Button color="primary" className="w-64 py-2 font-bold bg-primary text-white hover:bg-white hover:text-primary">
        {t("show_more")}
        </Button>
      </div>
    </div>
